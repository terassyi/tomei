...
// TestCueEcosystem_RealCueMod_SkipsOverlay verifies that when a cue.mod/ directory
// exists, the loader uses registry resolution instead of the virtual module overlay.
// Since we cannot reach the real OCI registry in unit tests, this test uses a manifest
// that does NOT use imports — it should still load successfully with cue.mod/ present.
func TestCueEcosystem_RealCueMod_SkipsOverlay(t *testing.T) {
	dir := t.TempDir()

	// Create cue.mod/module.cue (user-managed module)
...
`
	...
	// Manifest without imports (should work with cue.mod/ present)
...
gh: {
	...
	spec: {
		...
		version:      "2.86.0"
		source: url: "https://example.com/gh.tar.gz"
	}
}
`
...
// TestCueEcosystem_RealCueMod_TagsStillWork verifies that @tag() injection works
// correctly even when cue.mod/ exists (registry path).
func TestCueEcosystem_RealCueMod_TagsStillWork(t *testing.T) {
	dir := t.TempDir()
...
`
	...
	// Manifest with @tag() but no imports
...
`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.True(t, ok)
			assert.Equal(t, tt.expectedURL, tool.ToolSpec.Source.URL)
		})
	}
}
...
// TestCueEcosystem_BuildRegistry_CUERegistryEnvVar verifies that the CUE_REGISTRY
// environment variable is respected when building the registry.
func TestCueEcosystem_BuildRegistry_CUERegistryEnvVar(t *testing.T) {
	dir := t.TempDir()

	// Create cue.mod/ so the registry path is taken
	cueModDir := filepath.Join(dir, "cue.mod")
	require.NoError(t, os.MkdirAll(cueModDir, 0755))
	moduleCue := `module: "example.com@v0"
language: version: "v0.9.0"
`
	...
	// Simple manifest (no imports — registry won't actually be used for resolution)
	toolCue := `package tomei
...
`
	...
	// Set CUE_REGISTRY to a custom value (the actual value doesn't matter since
	// we're not resolving imports, but we verify the loader doesn't error)
	t.Setenv("CUE_REGISTRY", "tomei.terassyi.net=ghcr.io/terassyi,example.com=ghcr.io/example")
...
// TestCueEcosystem_BuildRegistry_DefaultFallback verifies that when CUE_REGISTRY
// is not set, the built-in default is used and loading succeeds.
func TestCueEcosystem_BuildRegistry_DefaultFallback(t *testing.T) {
	dir := t.TempDir()
...
`
	...
	require.NoError(t, err)
	require.Len(t, resources, 1)
	assert.Equal(t, "fd", resources[0].Name())
}

// TestCueEcosystem_GenerateCUERegistry_Integration verifies the full flow:
// cue.mod/ exists → GenerateCUERegistry returns the export statement.
func TestCueEcosystem_GenerateCUERegistry_Integration(t *testing.T) {
	dir := t.TempDir()

	// Create cue.mod/ in the directory
	cueModDir := filepath.Join(dir, "cue.mod")
	require.NoError(t, os.MkdirAll(cueModDir, 0755))
	...
	tests := []struct {
		...
		{
			name:         "posix with cue.mod",
			shell:        env.ShellPosix,
			wantContains: `export CUE_REGISTRY="tomei.terassyi.net=ghcr.io/terassyi"`,
		},
		{
			name:         "fish with cue.mod",
			shell:        env.ShellFish,
			wantContains: `set -gx CUE_REGISTRY "tomei.terassyi.net=ghcr.io/terassyi"`,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f := env.NewFormatter(tt.shell)
			result := env.GenerateCUERegistry(true, config.DefaultCUERegistry, f)
	...
	t.Run("no cue.mod", func(t *testing.T) {
		noCueModDir := t.TempDir()
		_ = noCueModDir
		f := env.NewFormatter(env.ShellPosix)
		result := env.GenerateCUERegistry(false, config.DefaultCUERegistry, f)
		assert.Empty(t, result)
	})
}

// TestCueEcosystem_VirtualOverlay_StillWorks verifies that manifests without cue.mod/
// continue to work through the virtual overlay path (backward compatibility).
func TestCueEcosystem_VirtualOverlay_StillWorks(t *testing.T) {
	dir := t.TempDir()

	// No cue.mod/ — should use virtual overlay
	content := `package tomei

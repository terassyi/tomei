...
_fdSource: {
	if _os == "linux" {
		url: "https://github.com/sharkdp/fd/releases/download/v\(_fdVersion)/fd-v\(_fdVersion)-\(_archMap[_arch])-unknown-linux-gnu.tar.gz"
	}
	if _os == "darwin" {
		url: "https://github.com/sharkdp/fd/releases/download/v\(_fdVersion)/fd-v\(_fdVersion)-\(_archMap[_arch])-apple-darwin.tar.gz"
	}
	if _os == "linux" && _arch == "arm64" {
...
_batSource: {
	if _os == "linux" {
		url: "https://github.com/sharkdp/bat/releases/download/v\(_batVersion)/bat-v\(_batVersion)-\(_archMap[_arch])-unknown-linux-gnu.tar.gz"
	}
	if _os == "darwin" {
		url: "https://github.com/sharkdp/bat/releases/download/v\(_batVersion)/bat-v\(_batVersion)-\(_archMap[_arch])-apple-darwin.tar.gz"
	}
	if _os == "linux" && _arch == "arm64" {
